...
function log_last_buttons_pressed()
	...
	for i = 1, #buttons_pressed do
		if
			table.find(last_buttons_pressed, buttons_pressed[i]) == nil
			and (buttons_pressed[i] > 0 and buttons_pressed[i] < 1024)
		then
	...
	huge_bubble(
		...
		MOUSE_Y,
		"ButtonLogUtils",
		"Use this for determining the ALT selector button assigned to your Honeycomb Bravo by X-Plane.",
		"Just turn the left selector knob to ALT and note the number.",
		"Last button pressed: " .. last_button_pressed
	)
end
