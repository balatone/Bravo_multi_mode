...
local function detect_rotary_event_from_bytes(prev_byte, cur_byte)
  if not prev_byte then return nil end
  prev_byte = prev_byte or 0
  cur_byte = cur_byte or 0
  if is_rising(prev_byte, cur_byte, ROTARY_PULSE_CW_MASK) then return "cw" end
  if is_rising(prev_byte, cur_byte, ROTARY_PULSE_CCW_MASK) then return "ccw" end
  return nil
end
...
local function detect_trim_event_from_bytes(prev_byte, cur_byte)
  ...
  if bit.band(cur_byte, TRIM_DOWN_MASK) ~= 0 then return "down" end
  if bit.band(cur_byte, TRIM_UP_MASK) ~= 0 then return "up" end
  -- If neither bit is set in current (we observed a falling edge), fall back to previous state
  if bit.band(prev_byte, TRIM_DOWN_MASK) ~= 0 then return "down" end
  if bit.band(prev_byte, TRIM_UP_MASK) ~= 0 then return "up" end
  return nil
end
...
function M.diagnostics()
  return { counters = counters, last_report = last_report, last_rotary_time = last_rotary_time, last_selector_time = last_selector_time, last_trim_time = last_trim_time, last_seen_rotary_byte = last_seen_rotary_byte, last_seen_selector_byte = last_seen_selector_byte }
end
